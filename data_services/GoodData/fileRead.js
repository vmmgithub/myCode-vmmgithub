...
exports.convertToCsv = function convertToCsv(inFile, outFile, entity, logFolder, dateConfig, tenant) {
    jsonPathsFromFile = new Array();
    createdOn.setFullYear(1723, 10, 3);
    lastModifiedOn.setFullYear(1723, 10, 3);

    fs.exists(tenant + '/internal/' + dateConfig + '.config', function (e) {
        if (e) {
            var dates = fs.readFileSync(tenant + '/internal/' + dateConfig + '.config', 'utf-8').replace('\n', '').split(',');
            if (dates[0] != '') {
    ...
    var header = getPathsAndHeader(tenant + '/internal/' + entity + '.map');
    var logStream = fs.createWriteStream(logFolder, {
        flags: 'a'
    });
    writeStream = fs.createWriteStream(csvFile + '.csv', {
        flags: 'w'
    });
    ...
    stream.on('end', function () {
        fs.writeFileSync(tenant + '/internal/dateLog_' + dateConfig + '.config', createdOn.toISOString() + ',' + lastModifiedOn.toISOString());
        var logText = "\n\n===============================================================";
...
function salesTage(obj) {
    ...
    switch (state) {
        case "Not Contacted": return "a. Not Contacted"; break;
        case "Contacted": return "b. Contacted"; break;
        case "Quote Requested": return "c. Quote Requested"; break;
        case "Quote Completed": return "d. Quote Completed"; break;
        case "Quote Delivered": return "e. Quote Delivered"; break;
        case "Customer Commitment": return "f. Customer Commitment"; break;
        case "PO Received": return "g. PO Received"; break;
        case "Closed Sale": return "h. Closed Sale"; break;
        case "No Service": return "i. No Service"; break;
        case "House Account": return "j. House Account"; break;
        case "Consolidated": return "k. Consolidated"; break;
        case "Transitioned": return "l. Transitioned"; break;
        default: return "XX. Error - Unknown Sales Stage"; break;
    };
};