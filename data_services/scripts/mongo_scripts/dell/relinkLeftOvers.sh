...
var dlStart = ISODate();
var dlName = '$BUID - $TYPE - Retry Linking';
var badQCount = 0;
var filter = { 
...
'systemProperties.expiredOn': ISODate('9999-01-01T00:00:00Z'),
 ...
 'systemProperties.qRank': 0
};

var cursor = db.$COLL.find(filter).limit($LIMIT).addOption(DBQuery.Option.noTimeout);
...
if [ ! -z DLID ]
then
curl -H'Content-Type:application/json' -u 'bill.moor@dell.com:passwordone' "https://dell-prd1dl2-int.ssi-cloud.com/rest/api/dell/app.dataloads/5144fbb0cee818f61651ab42::postProcess" -d '{"parallelChops": 64}'
fi
echo "---- Done" >> $LOG
echo "$DLID
" >> $DLIDTXT
