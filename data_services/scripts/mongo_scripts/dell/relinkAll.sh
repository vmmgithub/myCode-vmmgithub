...
var prevDLIds = [$DLIDS];
if (prevDLIds.length > 0) {
 filter['externalIds.id'] = {\$nin: prevDLIds};
}
...
if [ ! -z DLID ]
then
#curl --insecure -H'Content-Type:application/json' -u 'bill.moor@dell.com:passwordone' "https://dell-prd1dl2-int.ssi-cloud.com/rest/api/dell/app.dataloads/$DLID::postProcess" -d '{"parallelChops": 64}' 2>/dev/null
echo 'Done'
fi
echo "---- Done" >> $LOG
echo "$DLID
