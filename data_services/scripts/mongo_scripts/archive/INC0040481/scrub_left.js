...
var Scrub = function(data){
    ...
    var findOpportunity = function(displayName,callback){
        opportunityCollection.find({displayName:displayName},{start:1,limit:1},function(err,opportunities){
            if(err){
                callback(err);
            }else{
...
csvHelper.readAsObj("./csv/app_opportunities_left.csv",function(data){
