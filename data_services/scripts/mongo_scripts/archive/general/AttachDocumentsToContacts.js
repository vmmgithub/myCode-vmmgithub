...
commander.version("1.0")
    ...
    .option("-c, --compare [s]","Specify compare")
...
function Scrub(dryRun){
    ...
    var compare = (commander.compare && (commander.compare == "equal")) ? commander.compare : "regex";
    ...
    var findContacts = function(document,callback){
        ...
        var findPart = function(){
            var filter = {
                "extensions.tenant.customerNumber.value": {"$regex": "^" + document},
    ...
    var uploadDocument = function(filePath,callback){
        try{
            tenant.attachment(fs.createReadStream(filePath),function(error,response,body){
                if(error){
                    callback(error,null);
                }else{
                    try{
    ...
    return{
        ...
        checkDocumentContacts : function(){
            readDocumentDir(function(err,files,nextDir){
                    ...
                    function(file,next){
                        ...
                        findContacts(customerNumber,function(err,records){
                            if(err){
                                ...
                                uploadDocument(DOCUMENTS_DIR + "/" + file,function(err,documentObj){
                                    if(err){
