...
var RESELLER_ADMIN = 'disti.admin@' + RESELLER_TENANT + '.com';
...
var RESELLER_DATA = {
    ...
    "adminFirstName":"Distributor",
    "adminLastName":"Admin",
    "parentTeam":{"name" : "accountHeadTeam" },
    "parentRole":{"name" : "distributorTeams" },
    template: { name : 'cpo-template' }
};

if (conf.resellerName == 'osys')
    RESELLER_DATA.parentRole = { "name" : "channelPartnerTeams" };
...
console.log('addDistributor started for distributor ' + RESELLER_NAME);
...
async.waterfall([
    ...
    function(_reseller, cb) {
        ...
        if(!_reseller) {
            console.log('creating distributor organization ' + RESELLER_NAME);
            var doc = {
                name: RESELLER_NAME,
            ...
            reseller = _reseller;
            console.log('got existent distributor organization id ' + reseller._id + ', name ' + RESELLER_NAME);
            cb();
        }
    },
    function(cb) {
    ...
    function(cb) {
        ...
        api._doRequest('POST', '/rest/api/' + tenant + '/core.contacts::generatepolicies?synccall=Y', null, '{}', cb);
    },
    ...
    function(res, cb) {
        ...
        try {
            api._doRequest('POST', '/rest/api/' + tenant + '/refreshacl', null, '{}', cb);        
        } catch(e) {
            console.log("Error " + e);
            cb();
        }
    ...
    function(res, cb) {
        ...
        resApi = new Api({
            ...
            user: 'disti.admin@' + RESELLER_TENANT + '.com',
            pass: 'welcome'
        });
    ...
    function(cb) {
        ...
        //var data = { options: { force: true } };
        resApi._doRequest('POST', '/rest/api/' + RESELLER_TENANT + '/refreshacl', null, '{}', cb);
    },
    ...
    function(cb) {
        ...
        console.log('*** addDistributor done ***');
        cb();
    }
