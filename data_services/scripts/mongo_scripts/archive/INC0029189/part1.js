        ...
        _.each(oppToRep, function (repName, oppName) {
            function getSRR(cb) {
                ...
                if (!srr) {
                    ...
                    if (srr == DUMMY) {
                        console.log("Error: Opp " + oppName + " could not be updated as the sales rep " + repName + " could not be found. Err ");
                        cb(null, null);
                    } else {
                        cb(null, srr);
                    }
            ...
            function updateOpp(rel, oppRec, cb) {
                if (rel && oppRec) {
                    ...
                    if (!dryRun) {
                        oppColl.update(oppRec._id, { _id:oppRec._id, relationships:nRels}, function (err, res) {
                            ...
                            console.log("Success: " + oppName + " updated with salesRep " + rel.target.displayName);
                            cb(null);
                        });
                    }else{
                        console.log("Success: " + oppName + " updated with salesRep " + rel.target.displayName);
...
csvHelper.read("./csv/TOFIX.csv", function (data) {
    async.waterfall([
        function (callback) {
