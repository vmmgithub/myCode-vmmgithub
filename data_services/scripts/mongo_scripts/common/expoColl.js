...
//   Custom mapping   Adding custom columns in default list
if (!_.isEmpty(addCols)) {
   
    arrayAddCols=addCols.split(',');

  _.each(arrayAddCols, function(colString) {

    var collName = colString.split('|')[0];
    var collCol = colString.split('|')[1].toString();

    switch (collName) {
      case 'opportunities': COLL_LOOKUP[coll].push(collCol); break;
      case 'quotes': COLL_LOOKUP[coll].push(collCol); break;
      case 'bookings': COLL_LOOKUP[coll].push(collCol); break;
      case 'offers': COLL_LOOKUP[coll].push(collCol); break;
      case 'assets': COLL_LOOKUP[coll].push(collCol); break;
      case 'lineitems': COLL_LOOKUP[coll].push(collCol); break;
    }
