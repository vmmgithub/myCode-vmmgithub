...
var input = require('optimist')
    ...
    .alias('l', 'limit').describe('l', 'Concurrent threads').
default ('l', 5)
...
var updateOpps = function (opp, acrAmount, cb) {
    if (!opp.extensions.tenant.acramount) {
        opp.extensions.tenant.acramount = {
            value: acrAmount
        };
    }

 	console.log("Opp1 => ", opp.extensions.tenant.acramount.value);
 	console.log("Off => ", acrAmount);
//    if (opp.extensions.tenant.acramount.value) {

    	opp.extensions.tenant.acrmount.value = acrAmount;
    	oppCollection.update(opp, cb);
//       }
//    else cb;
       console.log("Opp2 => ", opp.extensions.tenant.acramount.value);
}
    ...
    scanQuotes(opp, function (err, quotes) {
        ...
        scanOffers(quotes, opp, function (err, offers) {
            ...
            _.each(offers, function (r) {
                ...
                else  {console.log(" Else listPrice==>", r.extensions.tenant.acramount.value);	
                        acrAmount += Number(r.extensions.tenant.acramount.value);}
...
findOpps(function (err, opps) {
    ...
    async.eachLimit(opps, 3, processOpps, function (err, res) {
