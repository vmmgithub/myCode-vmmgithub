...
//        if (_.isString(input.criteria)) input.criteria = JSON.parse(input.criteria);
        if (_.isString(input.criteria)) objCriteria = h.strToObj(objCriteria,input.criteria);
        callback();
    } catch (err) {
        callback(err);
    }
...
//        _.extend(genPayload, input.criteria);
...
var scanGeneratedOpportunities = function(tag, callback) {
    h.findRecords(opportunityCollection, {
        ...
        _.each(opportunities, scanOppsForReqFields);
        return callback();      
    });
};
