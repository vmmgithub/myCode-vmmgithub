...
for data_file in $@; do
	...
	#
	# Determin correct Line Terminator
	#
	if [[ "$(grep -l "$" $data_file)" == "$data_file" ]]
	then
		LINE_END="'\r\n'"
	...
	for SEGMENT in $TMP_DIR/$PREFIX*; do
		...
		then
			DATA_LOAD_STRING="load data $IS_LOCAL infile '$SEGMENT' into table
				$SCHEMA.ENTITLEMENT_DAILY_ACTIVITY
				fields terminated by '|' optionally enclosed by '\"'
				lines terminated by $LINE_END ignore $IG_NUM lines (
			AGREE_ID,
			AGREE_NAME,
			...
			THEATER,
			AGREE_SO,
			@t_NO_OF_ASSET_ENTL_ON_LINE,
			AGREE_LINE_ID,
			@t_AGREE_LINE_NUM,
			@t_AGREE_LINE_PART_LIST,
			...
			AGREE_ACCNT_ID,
			AGREE_ACCNT_NAME,
			AGREE_MASTER_ACCNT_ID,
			AGREE_MSTR_ACCNT_NAME,
			AGREE_ACCNT_GAN,
			...
			AGREE_END_CUST_ID,
			AGREE_END_CUST_MSTR_ID,
			AGREE_END_CUST_NAME,
			AGREE_END_CUST_MSTR_NAME,
			...
			AGREE_RES_MASTER_ACCNT_ID,
			AGREE_RES_ACCNT_NAME,
			AGREE_RES_ACCT_MSTR_NAME,
			AGREE_RES_ID,
			AGREE_RES_ADDRESS_1,
			AGREE_RES_ADDRESS_2,
			...
			NO_OF_ASSET_ENTL_ON_LINE=NULLIF(@t_NO_OF_ASSET_ENTL_ON_LINE,''),
			AGREE_LINE_NUM=NULLIF(@t_AGREE_LINE_NUM,''),
			AGREE_LINE_PART_LIST=NULLIF(@t_AGREE_LINE_PART_LIST,''),
			AGREE_LINE_PART_NET=NULLIF(@t_AGREE_LINE_PART_NET,''),
			AGREE_LINE_PART_NET_PER_ASSET=NULLIF(@t_AGREE_LINE_PART_NET_PER_ASSET,''),
			AGREE_LINE_QTY=NULLIF(@t_AGREE_LINE_QTY,''),
