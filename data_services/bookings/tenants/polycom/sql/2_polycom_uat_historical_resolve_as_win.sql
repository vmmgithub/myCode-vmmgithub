...
FROM
	...
	FROM
		...
		LEFT OUTER JOIN Polycom_UAT.APP_OFFERS offers
...
FROM
	polycom_data.ENTITLEMENT_DAILY_ACTIVITY_HISTORICAL eda
	INNER JOIN Polycom_UAT.APP_OFFERS offers
	ON eda.PREV_AGREE_ID = offers.EXTENSIONS_TENANT_AGREEID_VALUE
	AND eda.PREV_ENTL_SRVC_PART_NUM = offers.EXTENSIONS_TENANT_BUNDLEDSERVICEPARTNUMBER_VALUE
	AND eda.ASSET_ID = offers.EXTENSIONS_TENANT_ASSETID_VALUE
	AND eda.PREV_ENTITLEMENT_ID = offers.EXTENSIONS_TENANT_ENTITLEID_VALUE
...
SELECT DISTINCT
	...
	COALESCE(eda.AGREE_LINE_CURRENCY, eda.SO_CURRENCY, ''),
...
FROM
	...
	INNER JOIN Polycom_UAT.RELATIONSHIPS quote_r
	...
	INNER JOIN Polycom_UAT.RELATIONSHIPS opp_r
...
FROM
	...
	INNER JOIN Polycom_UAT.RELATIONSHIPS offer_r
...
/*
	AND opp_e.OPP_ID = (
		SELECT DISTINCT
			_ID
		FROM
			Polycom_UAT.APP_OPPORTUNITIES opp
		WHERE
			opp.#####
*/
GROUP BY
